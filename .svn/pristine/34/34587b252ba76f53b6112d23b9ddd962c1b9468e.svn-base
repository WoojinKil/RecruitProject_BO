...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addMulti = function(rowid,pos,irow,checked){
			...
			var	v = "<div class=\"tableCheck\"><label class=\"container type_along\" for=\"jqg_"+ts.p.id+"_"+rowid+"\"><input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+ts.p.id+"_"+rowid+"\"" + (checked ? "checked=\"checked\"" : "")+"/><span onclick=\"grid_stop_propagation(event, this); \" class=\"checkmark\"></span></label></div>",
		...
		updatepager = function(rn, dnd) {
			...
			// 2019.08.14 조회 시 clearGridData 하고 데이터 addJSONData 하는데  gridComplete이벤트가 두번 발생해 수정
			// clearGridData 만 dnd = false <-> 나머지 dnd = true  (나머지 = addRowData, loadState, addXmlData, addJSONData)
			// if($.isFunction(ts.p.gridComplete)) {ts.p.gridComplete.call(ts);}
			if($.isFunction(ts.p.gridComplete) && dnd ) {ts.p.gridComplete.call(ts);}
		...
		if(this.p.multiselect) {
			...
			this.p.colNames.unshift("<div class=\"tableCheck\"><label class=\"container type_along\" for='cb_"+this.p.id+"'><input  role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox'/><span class=\"checkmark\"></span></label></div>");
		...
		firstr = null;
		var hTable = $("<table class='ui-jqgrid-htable' style='width:"+ts.p.tblwidth+"px' role='presentation' aria-labelledby='gbox_"+this.id+"' cellspacing='0' cellpadding='0' border='0'><caption></caption></table>").append(thead),
...
$.jgrid.extend({
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(gi) {
						...
						v = "<div class=\"tableCheck\"><label class=\"container type_along\" for=\"jqg_"+t.p.id+"_"+rowid+"\"><input role=\"checkbox\" type=\"checkbox\""+" id=\"jqg_"+t.p.id+"_"+rowid+"\" class=\"cbox\" /><span class=\"checkmark\" onclick=\"grid_stop_propagation(event, this); \"></span></label></div>",
	...
	setFrozenColumns : function () {
		return this.each(function() {
			if ( !this.grid ) {return;}
			var $t = this, cm = $t.p.colModel,i=1, len = cm.length, maxfrozen = -1, frozen= false;/* JQGRIDCRUD이 무조건 추가되므로 i 값을 0에서 1로 변경 - frozen */
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10) + 3)+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
...
$.jgrid.extend({
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			//if (!$t.p.knv) {$($t).jqGrid("GridNav");} /* goodbug fix */
