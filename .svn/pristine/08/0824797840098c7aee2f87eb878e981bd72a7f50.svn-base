...
//                timeHtml = '<span class="fc-time">' + util_1.htmlEscape(timeText) + '</span>';
        ...
        if((util_1.htmlEscape(eventDef.title || '') || '&nbsp;').length < 30) {
            titleHtml =
                '<span class="fc-title">' +
                    (util_1.htmlEscape(eventDef.title || '') || '&nbsp;') + // we always want one line of height
                    ' </span><div class="fc-title-hover small">' + (util_1.htmlEscape(eventDef.title || '') || '&nbsp;') + '</div>';
        } else {
            titleHtml =
                '<span class="fc-title">' +
                    (util_1.htmlEscape(eventDef.title || '') || '&nbsp;') + // we always want one line of height
                    ' </span><div class="fc-title-hover">' + (util_1.htmlEscape(eventDef.title || '') || '&nbsp;') + '</div>';
        }
        return '<a class="' + classes.join(' ') + '"' +
            (eventDef.url ?
                ' href="' + util_1.htmlEscape(eventDef.url) + '"' :
