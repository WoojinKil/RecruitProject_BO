...
$(document).ready(function() {
	...
	var adm_grid_config = {
		...
		columns	    : [{name : 'USR_ID', label :'사번', sortable : false, editable : false},
			           {name : 'LGN_ID', label : '아이디', hidden : true},
			           {name : 'USR_NM', label : '이름', align : 'center', sortable : false, editable : false},
			           {name : 'ORG_NM', label : '조직명', align : 'left', sortable : false, editable : false},
// 			           {name : 'USR_EML_ADDR', label : '이메일', align : 'left', sortable : false, editable : false},
		...
		events		: {
			ondblClickRow : function(event, rowIdx, irow, icol) {
                ...
                if(isNotEmpty(row.USR_ID)) {
                	...
                	// 맵핑 메뉴 목록 조회
                	adm_mnu_grid.retreive({data:{usr_id:curr_usr_id, sys_cd: $("#sys_info").val()}});
                }
            },
		...
		events		: {
			ondblClickRow : function(event, rowIdx, irow, icol) {
                ...
                if(isNotEmpty(row.USR_ID) && isNotEmpty(row.MNU_SEQ) && isNotEmpty(row.PGM_ID) && adm_mnu_grid.getColName(icol) != "cb") {
                	...
                	curr_mnu_nm = row.MNU_NM;
                	curr_sys_cd = row.SYS_CD;
                	curr_sys_nm = row.SYS_NM;
    ...
    $("#btn_menu_save2").click(function(e) {
    	...
    	$("input[name=btnInf]:checked").each(function () {
    		...
    		if(btn_inf_nm != "") btn_inf_nm += ",";
    		btn_inf_nm += $(this).siblings("label").text();
		...
		if(!chg_flag) {
			...
			var obj = {
				 usr_id         : curr_usr_id
				, mnu_seq       : curr_mnu_seq
				, mnu_nm        : curr_mnu_nm
				, sys_cd       : curr_sys_cd
				, sys_nm        : curr_sys_nm
				, pgm_id        : $("#h_pgm_id").val()
				, mpp_btn_inf   : btn_inf
				, mpp_btn_inf_nm: btn_inf_nm
			}

		    ajax({
				url:'/psys/savePsys1029AdmMnuBtnList',
				data: obj,
	...
	for(var i = 0; i < list.length; i++) {
		...
		if(!isExist) {
			adm_mnu_grid.add({
				...
				SYS_CD    : list[i].SYS_CD,
				SYS_NM     : list[i].SYS_NM
			});
		} else {
...
function getPgmDtlInf(row) {
	...
	// 프로그램 상세 조회
	ajax({
		url : '/psys/getPsys1029AdmMnuBtnInf',
		data : obj,
		success:function(data) {
			if(_boolean_success == data.RESULT) {
				var pgmDtlInf = data.TsysAdmMnuBtnRtnnInf;
				if(isNotEmpty(pgmDtlInf)) {
					...
					for(var i in sp_btn_inf) {
						...
						if(sp_mpp_btn_inf.length > 0) {
							...
							$("#btnInf_" + sp_btn_inf[i]).prop("checked", true);
						}
					}
				}else {
					// 프로그램 상세 입력폼 초기화
