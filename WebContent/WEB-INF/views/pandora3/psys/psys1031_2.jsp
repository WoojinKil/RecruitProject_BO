...
$(document).ready(function(){
	...
	var rol_grid_config = {
			...
			columns	  : [
				          {name :'STATUS', label:'상태', align:'center', editable:false},
				          {name : 'UP_ORG_CD', label : '조직번호', hidden:true},
				          {name : 'ROL_ID', label : '권한ID', editable : false, align : 'center', sorttype : 'int', hidden:true},
				          {name : 'ROL_NM', label : '권한명', editable : false, align : 'left', required : true, edittype : 'text',  editoptions : {maxlength:300}},
				          ...
				          {name : 'F_APL_ST_DT', label : '적용시작일자', align : 'center', editable : false, required : true}, 
				          {name : 'F_APL_ED_DT', label : '적용종료일자', align : 'center', editable : false, required : true},
				          {name : 'SYS_CD', label : '사이트 이름', align : 'center', editable : false, edittype : 'select', formatter : 'select', editoptions : {value:'<c:out value="<%=CodeUtil.getSitGridComboList()%>" />'}, required : true}
	                     ],
	        ...
	        selecturl	: '/psys/getPsysOrgRolRtnn1031List.adm',	// 그리드 조회 URL
	        saveurl		: '/psys/savePsys1031.adm',				// 그리드 입력/수정/삭제 URL
	        search      : true,
			events		: {}
		};
	    
	    role_grid = new UxGrid(rol_grid_config);
	    role_grid.init();
	    role_grid.setGridWidth($('.table_org').width());
	...
	var adm_grid_config = {
		...
		columns	    : [{name : 'USR_ID', label :'사용자ID', hidden : true},
			           {name : 'UP_ORG_CD', label :'조직SEQ', hidden : true},
	...
	// 권한 조회 버튼 클릭 이벤트 (팝업 호출)
	$("#btn_rol_add").on("click", function () {
		...
		bpopup({
			...
			, params	: {callback : "fn_getRolList", target_id : _menu_id}
			, title		: "권한검색팝업"                          
			, type		: "l"
			, id        : "psys1031p1"
		});	
    ...
    // 조직별 권한 목록 그리드 - 삭제 버튼 클릭
    $("#btn_rol_del").click(function() {
    	role_grid.remove();
    });
    
	// 조직별 권한 목록 그리드 - 저장 버튼 클릭
    $("#btn_rol_save").click(function() {
    	role_grid.save();
    });
...
function fn_onClick(event, id)
{
	...
	//조직별 권한 목록 조회
	role_grid.retreive({data:{org_cd:org.id}, success : function() {
		// 선택한 조직의 구성원 조회
...
function fn_getOrgTree(org_cd) {
	...
	$.ajax({
		url			: _context + '/psys/getPsys1031List01',
		type		: 'POST',
		cache		: false,
 ...
 * 시스템 회원 목록 팝업
 ******************************/
//시스템 회원 목록 POPUP Callback
function fn_getUserList(user_list) {
	var user_list = user_list || {}
	var org_cd = $("#org_cd").val();
	
	for(var i = 0; i < user_list.length; i++) {
		var usrId = user_list[i].USR_ID;
		var isExist = false;
		
		if(adm_grid.getRowIndexByValue("USR_ID", usrId).length > 0) isExist = true;
		...
		if (!isExist) {
			adm_grid.add({
				...
				USR_STAT_CD	: user_list[i].USR_STAT_CD,
				ACTV_YN  	: user_list[i].ACTV_YN
			});
		}else {
			alert("이미 존재하는 회원입니다.");
		}
 ...
 * 조직별 권한 목록 팝업
 ******************************/
//조직별 권한 목록 POPUP Callback
function fn_getRolList(role_list) {
	...
	for(var i = 0; i < role_list.length; i++) {
		var rolId = role_list[i].ROL_ID;
		var sysCd = role_list[i].SYS_CD;
		
		
		if(role_grid.getRowIndexByValue("ROL_ID", rolId).length > 0  ||role_grid.getRowIndexByValue("SYS_CD", sysCd).length > 0) {
			isExist = true;
		} else  {
			role_grid.add({
				ROL_ID			: rolId,
				ORG_CD		: org_cd,
				ROL_NM			: role_list[i].ROL_NM,
				US_YN			: role_list[i].US_YN,
				F_APL_ST_DT		: role_list[i].F_APL_ST_DT,
				F_APL_ED_DT		: role_list[i].F_APL_ED_DT,
				SYS_CD			: role_list[i].SYS_CD,
			});
		}
	}
	if(isExist) {
		alert("중복 권한을 제외한 추가가 완료되었습니다.");
	}
					...
					<div class="detailInfo">
						...
						<div class="grid_right_btn">
							<div id="btn_menu_grid" class="grid_right_btn_in">
								<button class="btn_common btn_default" id="btn_rol_add">행추가</button>
								<button class="btn_common btn_default" id="btn_rol_del">행삭제</button>
								<button class="btn_common btn_default" id="btn_rol_save">저장</button>
							</div>
						</div>
