...
$(document).ready(function(){
    ...
    $("#rol_id").on("keydown", function (e) {
    	...
    	if(e.keyCode === 13) {
    		...
    		ajax({
    			url 	: "/psys/getSample1018RolList",
    			data 	: {rol_id:rol_id}, 
    			success : function (data) {
    				
    				if (data.RESULT == "S") {
    					var rolListCnt = data.rows.length;
    					if (rolListCnt > 1) {
    						
    						fn_open_bpopup(rol_id);
    						
    					} else if (rolListCnt == 1) {
    						$("#rol_nm").val(data.rows[0].ROL_NM);    						
    						$("#rol_id").val(data.rows[0].ROL_ID);    						
...
function fn_open_bpopup(rol_id) {
	
	bpopup({
		  url       :"/psys/forward.sample1018p001.adm"
		, params	: {callback : "fn_sample1018p001_callback", target_id : _menu_id, rol_id : rol_id}
		, title		: "권한 조회"                          
			, type		: "l"
		, height	: "450px"
		, id        : "sample1018p1" 
...
function fn_sample1018p001_callback(row) {
	
	$("#rol_nm").val(row.ROL_NM);    						
	$("#rol_id").val(row.ROL_ID);    
			...
			<form name="search" id="search" name="search" onsubmit="return false">
	            ...
	            <table class="tblType1 mB60" >
		            ...
		            <tr>
		                ...
		                <td>
		                	<span class="txt_pw "> 
		                  		<input type="text" name="rol_id" id="rol_id" class="text w30" value="" />
		                  		<!-- <input type="button" class="btn_common btn_default" id="btn_rol_search" value="검색버튼" > -->
		                  		<button type="button" class="btn_common btn_default" id="btn_rol_search">
									<img src="${pageContext.request.contextPath}/resources/pandora3/images/common_new/bg_magnify.png" alt="검색버튼" />
								</button>
		                  		<input type="text" name="rol_nm" id="rol_nm" class="text w30" value="" disabled="disabled"/>
	                    	</span>
		                </td>                
		                <th>사용여부</th>
		                <td>
